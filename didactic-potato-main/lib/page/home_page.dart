...
class HomePage extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<Article>>(
          ...
          builder: (context,snapshot)=> switch(snapshot.connectionState){
            ConnectionState.done when snapshot.data != null => ListArticles(listArticles: snapshot.data!),
          ConnectionState.waiting => Center(child: const CircularProgressIndicator()),
          _ => Icon(Icons.error)
          }
      ),
    );
  ...
  Future<List<Article>> fetchListProducts() async{
    ...
    //on telecharge la donnee
    Response resListArt= await get(Uri.parse(uri));
    //si la requete s'est bien passee et que le corps de la reponse
    // n'est pas vide
    if (resListArt.statusCode == 200 && resListArt.body.isNotEmpty){
      ...
      throw Exception("Requete invalide");
    }
  }
}
